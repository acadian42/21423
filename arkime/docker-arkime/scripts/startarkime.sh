...
export ARKIME_INSTALL_DIR=/opt/arkime/raw

if [ ! -f $ARKIMEDIR/etc/.initialized ]; then
    echo -e "$ARKIME_LOCALELASTICSEARCH\n$ARKIME_INET" | $ARKIMEDIR/bin/Configure
    echo INIT | $ARKIMEDIR/db/db.pl --insecure https://$OS_HOST:$OS_PORT init
...
then
    ...
    then
        ...
        exec $ARKIMEDIR/bin/capture --insecure --config $ARKIMEDIR/etc/config.ini --host $ARKIME_HOSTNAME >> $ARKIMEDIR/logs/capture.log 2>&1 &
    else
...
then
    ...
    exec $ARKIMEDIR/bin/node viewer.js --insecure -c $ARKIMEDIR/etc/config.ini --host $ARKIME_HOSTNAME >> $ARKIMEDIR/logs/viewer.log 2>&1
    popd
fi
